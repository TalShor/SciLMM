...
from scilmm import SciLMM_foo, load_sparse_csr
...
class TestSciLMM(TestCase):
    ...
    def test_circular_family_tree(self):
        assert_raises(NetworkXUnfeasible,
                      SciLMM_foo, fam='./Examples/Tests/circular_rel.fam', output_folder=self.output_folder)

        SciLMM_foo(fam=os.path.join(self.output_folder, 'circular_rel.fam'), output_folder=self.output_folder,
                   remove_cycles=True)
    ...
    def test_half_full_brothers(self):
        SciLMM_foo(fam=os.path.join(self.output_folder, 'half_and_full_brothers.fam'), output_folder=self.output_folder,
                   ibd=True)
        entries_list = np.load(os.path.join(self.output_folder, "entries_ids.npy"))
    ...
    def test_more_than_two_parents(self):
        SciLMM_foo(fam=os.path.join(self.output_folder, 'too_many_parents.fam'), output_folder=self.output_folder,
