...
def read_fam(fam_file_path=None, fam=None, null_value='0', female_value='2', phenotype_of_interest='1'):
    """
    ...
    if null_value:
        child_father = np.array([[entries[child], entries[father]] for child, father in
                                 df[['IID', 'F_IID']][df['F_IID'] != null_value].values])
        child_mother = np.array([[entries[child], entries[mother]] for child, mother in
                                 df[['IID', 'M_IID']][df['M_IID'] != null_value].values])
    else:
        child_father = np.array([[entries[child], entries[father]] for child, father in
                                 df[['IID', 'F_IID']][~df['F_IID'].isna()].values])
        child_mother = np.array([[entries[child], entries[mother]] for child, mother in
                                 df[['IID', 'M_IID']][~df['M_IID'].isna()].values])
    ...
    if "phenotype" in df.columns:
        if phenotype_of_interest:
            interest = np.array([entries[entry_id] for entry_id in df[df['phenotype'] == phenotype_of_interest]['IID']])
        else:
            interest = np.array([entries[entry_id] for entry_id in df[~df['phenotype'].isna()]['IID']])
    else:
        interest = np.array([])
    if interest.size == 0:
        interest = None
